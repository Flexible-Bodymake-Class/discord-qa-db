...
毎週火曜19:00にタスクスケジューラ（`FBC_QA_Auto_Update`）が `auto_export.ps1` を自動実行し、
Discordエクスポート→スプシ反映（①②）まで自動で完了する。ログは `logs/` フォルダに出力される。
人間がやるのは③の非公開判断のみ。

前提: 実行時刻にPCが起動していること（スリープ中は実行されない）。
トークンは `token.txt`（Git管理外）に保存されている。失効した場合は同ファイルを更新する。
